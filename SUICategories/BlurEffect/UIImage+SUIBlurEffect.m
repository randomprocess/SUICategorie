...
@implementation UIImage (SUIBlurEffect)
...
- (UIImage *)sui_imageByApplyingTintEffectWithColo:(UIColor *)tintColo
{
    ...
    size_t componentCount = CGColorGetNumberOfComponents(tintColo.CGColor);
    if (componentCount == 2) {
        CGFloat b;
        if ([tintColo getWhite:&b alpha:NULL]) {
            effectColor = [UIColor colorWithWhite:b alpha:EffectColorAlpha];
        }
    } else {
        CGFloat r, g, b;
        if ([tintColo getRed:&r green:&g blue:&b alpha:NULL]) {
...
#define ENABLE_TINT                     1
    ...
    {
        CGContextSaveGState(outputContext);
        CGContextSetFillColorWithColor(outputContext, tintColo.CGColor);
